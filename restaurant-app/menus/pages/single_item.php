...
<body>
    ...
    <div id="content">
                    ...
                    <?php if (count($items) > 0) : ?>
                        <?php foreach ($items as $item) : ?>
                            <p><?php echo htmlspecialchars($item['name']); ?> <span>(
                                <?= htmlspecialchars($currency) ?> <?php echo htmlspecialchars($item['price']); ?>/- x
            ...
            <section id="cartReload">
                ...
                <?php if (count($items) > 0) : ?>
                    ...
                    foreach ($items as $item) {
                        // Cart images - assets is in menus/, need only ../
        ...
        if (isset($_GET['food_item_id']) && filter_var($_GET['food_item_id'], FILTER_VALIDATE_INT)) {
            ...
            if ($result->num_rows > 0) {
                ...
                $more_details = htmlspecialchars($row['more_details'], ENT_QUOTES, 'UTF-8');
                // Convert newline characters to HTML <br> tags
                $more_details = nl2br($more_details);
                ...
                for ($i = 1; $i <= 4; $i++) {
                    ...
                    if (!empty($image_url)) {
                        // Fix image path - assets is in menus/, need only ../
                        if (strpos($image_url, '../') !== 0 && strpos($image_url, 'http') !== 0) {
                            $image_url = '../' . $image_url;
                        }
                        $images[] = $image_url;
        ...
        ?>
                <div class="container" style="margin-top: 12rem">
                                ...
                                <div class="carousel-inner">
                                    ...
                                    foreach ($images as $index => $image_url) {
                                        ...
                                        echo "<div class='carousel-item $active'>
                                            <img src='$image_url' class='d-block w-100' alt='Product Image " . ($index + 1) . "' onerror='this.src=\"../assets/imgs/placeholder.jpg\"' />
                                          </div>";
                                    }
                                    ?>
                                </div>
                                ...
                                <div class="row">
                                    ...
                                    foreach ($images as $index => $image_url) {
                                        echo "<div class='col-3'>
                                            <img src='$image_url' class='img-thumbnail' alt='Product Thumbnail " . ($index + 1) . "' data-bs-target='#productCarousel' data-bs-slide-to='$index' onerror='this.src=\"../assets/imgs/placeholder.jpg\"' />
                                          </div>";
                                    }
                                    ?>
                                </div>
                                ...
                                <?php if (!empty($video_link)): ?>
                                    <div class="ratio ratio-16x9">
                                        <video width="100%" height="100%" controls style="object-fit: cover;">
                                            <source src="<?php echo htmlspecialchars($video_link, ENT_QUOTES, 'UTF-8'); ?>" type="video/mp4">
                                            <source src="<?php echo htmlspecialchars($video_link, ENT_QUOTES, 'UTF-8'); ?>" type="video/webm">
                                            <source src="<?php echo htmlspecialchars($video_link, ENT_QUOTES, 'UTF-8'); ?>" type="video/ogg">
                    ...
                    <div class="product-details mt-4 fade-down">
                        <h3>More Details</h3>
                        <p class="short-description"><?php echo nl2br($more_details); ?></p>
                    </div>
    ...
    <script>
        ...
        function addToCart(foodItemId) {
            ...
            formData.append('table_number', '<?php echo htmlspecialchars($table_number); ?>');
